...
class Albuminfo():
    ...
    def __init__(self):
        ...
        for i in self.album_id:
                self.album_lst.append([str(i), str(self.album_info_dic[i]['albumtitle']) ,str(self.album_info_dic[i]['albumtype']), str(self.album_info_dic[i]['likecnt']), self.album_info_dic[i]['발매사'], self.album_info_dic[i]['기획사'], 
...
class Artistsinfo():    
    ...
    def Mysql_artists(self):         
        ...
        save(self.sql_truncate, self.sql_insert, self.artists_dic_t)

Melon = Melon()
print(Melon.MelTop100_table)
# Melon.Mysql_meltop()
# Melon.Mysql_song()
# Melon.Mysql_songrank()

# Albumn=Albuminfo()
# Albumn.Mysql_albuminfo()
